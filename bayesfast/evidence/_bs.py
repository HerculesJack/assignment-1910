...
def BS(xp=None, xq=None, logp=None, logq=None, logp_xp=None, logp_xq=None, 
       logq_xp=None, logq_xq=None, logp_vectorized=True, logq_vectorized=True, 
       logp_vectorize_level=1, logq_vectorize_level=1):
    ...
    logr_err = (np.var(_f1) / np.mean(_f1)**2 / n_q + tau * np.var(_f2) / 
                np.mean(_f2)**2 / n_p)**0.5
...
def WarpBS(xp, logp, logp_xp=None, logp_vectorized=True, logp_vectorize_level=1, 
           random_state=None):
    if logp_xp is not None:
    ...
    xq = rs.multivariate_normal(xp_mu, xp_cov, xp_0.shape[0])
